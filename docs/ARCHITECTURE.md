...
- `npm run dev` starts Vite at the fixed strict origin `http://127.0.0.1:5173`, builds main/preload, and launches Electron in a separate process group. Renderer edits use HMR. Application, main, preload, core, device, and DFU edits stage a rebuilt bundle; the development host never signals or force-kills a running Electron process, and restarts it only after the developer quits normally when safe. Stopping the host leaves Electron running under its own no-quit boundary and prints an explicit reminder to close it normally when safe.
